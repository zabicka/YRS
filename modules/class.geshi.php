...
//                }
    ...
    function finalise(&$parsed_code) {
        ...
        if ($this->add_ids && !$this->overall_id) {
            $this->overall_id = '';//'geshi-' . substr(md5(microtime()), 0, 4); // yetty remove
        }
        ...
        if ($this->line_numbers != GESHI_NO_LINE_NUMBERS && $this->header_type != GESHI_HEADER_PRE_TABLE) {
            ...
            for ($i = 0, $n = count($code); $i < $n;) {
                ...
                if ($this->add_ids) {
                    $attrs['id'][] = ''; //"$this->overall_id-$i"; // yetty remove
                }
    ...
    function header() {
         ...
         */
        $attributes = ' class="code ' . $this->language;
        ...
        if ($this->overall_id != '') {
            $attributes .= ''; //" id=\"{$this->overall_id}\""; // yetty remove
        }
