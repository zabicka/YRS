 ...
 */
function adresa() {
	return URL::create(func_get_args());
}

/**
 * funkce, ktera prijima nazev tridy a dodava prelozeny nazev. Kdyz je reverze
 * false, doda se v jazyce uzivatele a vraci se systemovy. Pokud je reverze true
 * doda se systemovy a vraci se prelozeny
 *
 * {@source}
 * @param string $vyraz nazev tridy, ke ktere se ma ziskat protejsek
 * @param boolean $reverze udava, zda se ma ziskat systemovy (true) nebo uzivatelsky (prelozeny - false)
 * @param string(2) $lang udava jazyk, nemusi se dodat, vezme se z <var>$_COOKIE["lang]</var>
 * @param string $class pokud se jedna po akci, urcuje v jake tride
 * @return string
 */
function slovnik($vyraz, $reverze = false, $lang = NULL, $class="") {
	global $db;

	if($lang=="") $lang = $_COOKIE["lang"];
	if($class!="") $qclass = " and class='".mysql_real_escape_string(slovnik($class))."'";

	if($reverze == false) {
		$dotaz = "select cim as ret from __dictionary where co='".mysql_real_escape_string($vyraz)."' and (jazyk='".$lang."' or jazyk='')".$qclass;
	} else {
		$dotaz = "select co as ret from __dictionary where cim='".mysql_real_escape_string($vyraz)."' and (jazyk='".$lang."' or jazyk='')".$qclass;
	}
	if($db!="")
		$ret = $db->fetch_array($dotaz);
...
/**
 * zjednodusene volani funkce slovnik pouze s vyrazem a tridou.
 */
function sslovnik($vyraz, $class=NULL) {
	return slovnik($vyraz, false, NULL, $class);
}

/**
 * vola funkci {@link slovnik()} se zapnutou reverzi
 */
function rslovnik($vyraz, $lang=NULL, $class="") {
	return slovnik($vyraz, true, $lang, $class);
}

/**
 * Funkce, ktera vytvari zaznamy pro modul v tabulce slovniku.
 *
 * {@source}
 * @param string $co Nazev tridy
 * @param array string $cim Pole s hodnotami -- array("jazyk"=>"nahrazka")
 * @return boolean
 */
function instalaceSlovniku($cim, $co, $jazyk="") {
	global $db;

	if(is_array($co)) {
		foreach($co as $jazyk=>$nahrazka) {
			$db->query("insert into __dictionary (co, cim, jazyk) values ('".$nahrazka."', '".$cim."', '".$jazyk."')");
		}
	} else {
		$db->query("insert into __dictionary (co, cim, jazyk) values ('".$co."', '".$cim."', '".$jazyk."')");
	}
	return true;
}
